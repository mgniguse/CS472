...
//  app.use("/", (req, res, next) => { // Note: app.use(...) always runs
...
// not handled by any other route handler middleware above. In 
// this hanlder we will redirect the user to 
// an error page with PAGE NOT FOUND message and status
// code as 404 (HTTP status code for Resource NOT found)
//  app.use((req, res) => { // Do NOT use app.use(...) bcos it always runs
 app.all("*", (req, res) => {
    console.log(`Responding with a 404 Error page`);
    res.status(404).sendFile(path.join(__dirname, "views", "404.html"));
 });
